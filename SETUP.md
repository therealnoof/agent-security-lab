...
| You edit `.env` but the agent still uses the old value | Stale `set -a; source .env; set +a` from earlier in the shell took precedence (older versions of compose). The repo's compose now uses `env_file:` to read `.env` directly | `unset CALYPSOAI_OPENAI_API_BASE CALYPSOAI_TOKEN CALYPSOAI_MODEL` (or open a fresh shell), then re-run the agent. If the lab repo on disk is older than this README, ask your instructor to refresh it |
...
| Sessions row in the UI shows `Tools: 0` even for runs that called MCP tools | The `toolCount` counter does not appear to be populated by OpenAI-protocol `tool_calls` flowing through the proxy; tenant has no surfaced "tool catalog" UI to register them. Behavior verified May 2026 — still unexplained | No client-side fix. Add to the same F5/Calypso support ticket alongside the fingerprint-renderer bug; both look like agentic-pipeline gaps for openai-compat providers |
| Same `sessionId` appears twice in `agent-sessions` API output with different `id` fields | The platform stores multiple records per session (likely one lifecycle, one fingerprint). Cosmetic, not functional | Add to the support ticket as a clarification question; it does not affect lab demos |
| New Agent project doesn't issue tokens or doesn't route traffic | The "Live Project" checkbox in project creation isn't documented in the public API docs but appears to set `deploymentStatus: "live"` on the project. Without it, the project may be in a draft-like state | Edit the project, check **Live Project**, save. Confirm with `curl …/backend/v1/projects` that `deploymentStatus: "live"`. Add to the support ticket alongside the other undocumented surfaces |
| Threat-Intel or Remediation crashes with `'list' object has no attribute 'choices'` (Calypso UI shows `response.json() is an array, can't index with string`) | Calypso's openai-compat response template assumes the upstream response is a JSON object; Gemini returns an array shape for some tool-calling responses, and the template breaks. Same Gemini-openai-compat family as the fingerprint-renderer and `toolCount` bugs | **Workaround**: configure a second provider in your Agent project pointed at a real OpenAI model (e.g. `gpt-4o-mini`) and update `.env`'s `CALYPSOAI_OPENAI_API_BASE` to use that provider. OpenAI providers don't trip this template bug. Add to the F5 support ticket |
...
1. **Create one Agent project per learner cohort** (or per learner if isolation matters). Click-by-click in the Calypso UI:
   ...
   4. Choose project type **Agentic**.
   5. Give it a name learners will recognize (e.g., `agent-security-lab-cohort-2026-05`).
   6. Select the **default model** for the project. This is the LLM provider every agent in this project will route through. For the lab, pick something that supports OpenAI Chat Completions + tool calling (e.g., a Grok provider configured for `/v1/chat/completions`, an OpenAI provider, etc.). See [§A6 Common self-paced gotchas](#a6-common-self-paced-gotchas) about Gemini's openai-compat shim and choose accordingly.
   7. **Check the "Live Project" box.** The Calypso API docs do not currently explain this toggle, but empirically a live project has `deploymentStatus: "live"` in its API record and serves traffic; without it you may not be able to issue tokens or route requests. Treat it as required until F5 documents it (open question in the support-ticket bundle below).
   8. Save.

   The project is the unit of scoping in Calypso — it owns the sessions, the API tokens, the guardrails, and the provider connections.
2. **Create per-learner API tokens** inside that project: open the project → **API Tokens** → **Create**. One token per learner. Give each a recognizable name (e.g., `lab-2026-05-02-learner-07`) and the lowest scope that still lets them call the OpenAI-compatible proxy and read their own session traces. The token value is what learners paste into `.env` as `CALYPSOAI_TOKEN`.
